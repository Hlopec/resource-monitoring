...
Stage `03.2` introduces the Resource collection query boundary without widening `ResourceRepository`; Stage `03.3.1` extends the same boundary with the explicit Resource details read method. The repository remains narrow and transactional: id lookup, canonical-name lookup, existence checks, explicit lock lookup, and add/flush-style persistence operations. It must not grow list, search, filter, paginate, sort, or generic query-spec methods. Collection and details reads use the dedicated application port `ResourceQueryService`, exposed inside the active Unit of Work as `uow.resource_queries`.
...
Stage `03.2.5` closes the Resource query-service baseline without adding a new query capability. Stage `03.3.1` then adds the Resource details read model as the next explicit query-service method. The audited workflow inventory is:
