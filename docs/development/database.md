...
On success, the handler closes only the existing current row with `valid_to = transitioned_at`, constructs exactly one replacement `ResourceState` with `valid_from = transitioned_at` and `valid_to = NULL`, adds it through the append/read repository, and updates the locked `resource` snapshot fields to match the new current state. It materializes `ResourceStateTransitionedResult(resource_id, previous_state_id, new_state_id, transitioned_at)` before committing. No explicit flush is needed because UUIDv7 ids are generated when mapped objects are constructed.
