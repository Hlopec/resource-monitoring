...
Stage `03.2.1` adds the first Resource collection read boundary through `ListResourcesHandler`, the `ResourceQueryService` application port, and the SQLAlchemy adapter `app.persistence.sqlalchemy.queries.resources.SQLAlchemyResourceQueryService`. `ResourceRepository` remains a narrow transactional repository and does not expose list, search, filter, pagination, arbitrary sort, or generic query-spec methods.

`ListResourcesQuery` carries explicit `tenant_id`, optional exact `resource_type_id`, optional exact `lifecycle_status_id`, `page_size`, and optional opaque `cursor`. Page size is validated before opening a Unit of Work with minimum `1`, default `50`, and maximum `200`. Organization filtering is intentionally deferred because Resource does not have a direct `organization_id`; organization ownership is temporal and requires a separate join policy.

Resource list pages are ordered by `resource.created_at ASC, resource.id ASC` and use keyset pagination. Version `1` cursors are URL-safe base64 JSON containing `created_at` and `id` for the last returned item. The SQL continuation predicate is `created_at > cursor_created_at OR (created_at = cursor_created_at AND id > cursor_id)`. The adapter applies mandatory tenant scope on every request, applies optional exact filters, selects only summary columns, executes one projection query with `LIMIT page_size + 1`, returns no `total_count`, and never emits `OFFSET` or `COUNT(*)`.

`ResourceSummaryResult` contains `resource_id`, `tenant_id`, `resource_type_id`, `lifecycle_status_id`, `canonical_name`, `display_name`, `record_version`, `first_seen_at`, `last_seen_at`, `created_at`, and `updated_at`. `ResourcePageResult` contains immutable page items, `next_cursor`, and `page_size`. Query handlers materialize these application DTOs before Unit of Work exit and do not commit, flush, mutate rows, lock resources, retry errors, cache pages, or expose SQLAlchemy objects.

The cursor is a position marker, not a snapshot. It does not include tenant or filter state; tenant isolation comes from the SQL predicate, so using a cursor with a different tenant cannot expose the previous tenant's rows. The current schema has tenant/filter indexes for Resource type and lifecycle-status lookups, but no composite `(tenant_id, created_at, id)` index for the default keyset order. Add that index in a follow-up migration before high-volume list usage depends on this path.
