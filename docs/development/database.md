...
Concrete Tenant, Organization, Label, Resource, managed catalog, temporal fact, and lineage adapters are available under `app.persistence.sqlalchemy.repositories`. Resource collection query adapters are available under `app.persistence.sqlalchemy.queries`. `SQLAlchemyUnitOfWork` exposes `uow.tenants`, `uow.organizations`, `uow.labels`, `uow.resources`, `uow.resource_types`, `uow.identifier_types`, `uow.relationship_types`, `uow.ownership_roles`, `uow.classification_types`, `uow.classification_values`, `uow.lifecycle_statuses`, `uow.criticalities`, `uow.exposure_levels`, `uow.resource_identifiers`, `uow.resource_ownerships`, `uow.resource_relationships`, `uow.resource_classifications`, `uow.resource_labels`, `uow.resource_states`, `uow.resource_aliases`, `uow.resource_merges`, and `uow.resource_queries` while the Unit of Work is active. These repositories and query services share the Unit of Work session and are unavailable before enter, after exit, and after the Unit of Work leaves the active state.

The final `03.1` Resource application core exposes explicit handlers for base Resource creation, Resource details reads by id and canonical name, state transition, identifier assignment, ownership assignment, relationship assignment, alias assignment, merge lineage recording, classification assignment, label assignment, and canonical Resource resolution. Read handlers use one Unit of Work, do not commit, do not take write locks, and materialize immutable results before exit. Write handlers use one Unit of Work, perform tenant-scoped checks, take explicit Resource locks where mutation requires them, materialize results before commit, and commit exactly once as the final meaningful persistence operation. No handler explicitly rolls back or retries; `SQLAlchemyUnitOfWork` owns cleanup and persistence error translation.

`AssignResourceRelationshipHandler` and `MergeResourceHandler` are the only current dual-Resource write handlers. They share the same deterministic UUID-string physical lock ordering policy while preserving semantic source and target roles in inserted rows and error metadata. Single-Resource mutation handlers lock only the requested Resource. Canonical resolution remains read-only and does not lock.

Resource details expose current state, current identifiers, current ownership, current classifications, current labels, aliases, and the direct outgoing merge edge. They intentionally do not expose outgoing or incoming relationships, and they do not perform canonical resolution. Canonical resolution remains a separate query so ordinary Resource reads do not silently rewrite caller intent.

Stage `03.2` adds the Resource collection read boundary through `ListResourcesHandler`, the `ResourceQueryService` application port, and the SQLAlchemy adapter `app.persistence.sqlalchemy.queries.resources.SQLAlchemyResourceQueryService`. `ResourceRepository` remains a narrow transactional repository and does not expose list, search, filter, pagination, arbitrary sort, or generic query-spec methods.

`ListResourcesQuery` carries explicit `tenant_id`, optional exact `resource_type_id`, optional exact `lifecycle_status_id`, optional exact `organization_id`, optional exact `label_id`, optional exact `classification_type_id`, optional exact `classification_value_id`, `page_size`, and optional opaque `cursor`. Page size is validated before opening a Unit of Work with minimum `1`, default `50`, and maximum `200`; a `classification_value_id` without `classification_type_id` is also rejected before Unit of Work creation. The organization filter means the exact organization referenced by the Resource's current primary `owner` `ResourceOwnership` row. Historical rows, current non-primary rows, and current primary rows for other ownership roles do not match the organization filter. `label_id` matches only current tenant-scoped `resource_label` assignments for that label id; historical assignments do not participate and there is no key/value search. `classification_type_id` alone matches any current `resource_classification` row of that type, including non-primary rows. `classification_type_id` plus `classification_value_id` matches the exact current pair. Nonexistent label or classification ids return empty pages. Organization hierarchy traversal, multi-organization filters, multi-label filters, OR/NOT syntax, facets, fuzzy search, and generic filter dictionaries remain out of scope.

Resource list pages are ordered by `resource.created_at ASC, resource.id ASC` and use keyset pagination. Version `1` cursors are URL-safe base64 JSON containing `created_at` and `id` for the last returned item; no filters are encoded in the cursor. The SQL continuation predicate is `created_at > cursor_created_at OR (created_at = cursor_created_at AND id > cursor_id)`. The adapter applies mandatory tenant scope on every request, applies optional exact filters with `AND`, selects only summary and current-primary owner scalar columns, executes one projection query with `LIMIT page_size + 1`, returns no `total_count`, and never emits `OFFSET`, `COUNT(*)`, or `DISTINCT`.

`ResourceSummaryResult` contains `resource_id`, `tenant_id`, `resource_type_id`, `lifecycle_status_id`, `canonical_name`, `display_name`, `primary_organization_id`, `primary_ownership_role_id`, `record_version`, `first_seen_at`, `last_seen_at`, `created_at`, and `updated_at`. `ResourcePageResult` contains immutable page items, `next_cursor`, and `page_size`. Query handlers materialize these application DTOs before Unit of Work exit and do not commit, flush, mutate rows, lock resources, retry errors, cache pages, perform per-Resource ownership lookups, or expose SQLAlchemy objects.

The adapter uses a left outer join from `resource` to the current primary `owner` `resource_ownership` row with tenant, resource, `ownership_role_id`, `is_primary IS true`, and `valid_to IS NULL` predicates. The seeded `owner` role is used because the database allows one current primary owner per ownership role; this preserves one row per Resource without `DISTINCT`. Resources without current primary ownership remain visible when `organization_id` is absent and expose `None` ownership projection fields. With `organization_id` present, only Resources whose current primary owner row references that exact organization match.

Label and classification filters use correlated `EXISTS` predicates rather than joins. The label predicate is `resource_label.tenant_id = :tenant_id`, `resource_label.tenant_id = resource.tenant_id`, `resource_label.resource_id = resource.id`, `resource_label.label_id = :label_id`, and `resource_label.valid_to IS NULL`. The classification predicate is `resource_classification.tenant_id = :tenant_id`, `resource_classification.tenant_id = resource.tenant_id`, `resource_classification.resource_id = resource.id`, `resource_classification.classification_type_id = :classification_type_id`, `resource_classification.valid_to IS NULL`, and, for type/value queries, `resource_classification.classification_value_id = :classification_value_id`. This preserves one Resource row per result even when a Resource has multiple current labels or multiple current classifications, without `DISTINCT` or application-side deduplication.

The cursor is a position marker, not a snapshot. It does not include tenant or filter state; tenant isolation comes from the SQL predicate, so using a cursor with a different tenant cannot expose the previous tenant's rows. The current schema has tenant/filter indexes for Resource type and lifecycle-status lookups; ownership indexes for tenant/resource, tenant/organization, tenant/role, tenant/validity, tenant/resource/role, current uniqueness, and current-primary uniqueness; ResourceLabel indexes `ix_resource_label_tenant_resource_label`, `ix_resource_label_tenant_label_id`, `ix_resource_label_tenant_valid_to`, and `uq_resource_label_current`; and ResourceClassification indexes `ix_resource_classification_tenant_resource_value`, `ix_resource_classification_tenant_resource_type`, `ix_resource_classification_tenant_value`, `ix_resource_classification_tenant_type_value`, `ix_resource_classification_tenant_valid_to`, `uq_resource_classification_current_value`, and `uq_resource_classification_current_primary_type`. It has no composite `(tenant_id, created_at, id)` index for the default keyset order, no partial label filter index like `(tenant_id, label_id, resource_id) WHERE valid_to IS NULL`, no partial classification filter index like `(tenant_id, classification_type_id, classification_value_id, resource_id) WHERE valid_to IS NULL`, and no partial organization-filtered current-primary owner index. Add follow-up indexes before high-volume list usage depends on this path.

Stage `03.2.3` also adds exact identity lookup through the same `ResourceQueryService` boundary. `FindResourceByIdentifierQuery` carries `tenant_id`, `identifier_type_id`, `namespace`, and `normalized_value`; `FindResourceByAliasQuery` carries `tenant_id`, `alias_type`, and `normalized_value`. Handlers reject blank string inputs before opening a Unit of Work, preserve accepted values exactly, and do not normalize, lowercase, parse, hash, or route through generic search.

Identifier lookup uses the current-row predicate `resource_identifier.valid_to IS NULL` plus exact tenant, identifier type, namespace, and normalized-value predicates. `namespace=None` means `namespace IS NULL`; non-null namespace values use exact equality. Historical identifiers are ignored, current non-primary identifiers are eligible, and matching uses `normalized_value`, not `original_value`. `value_hash` is not used in the lookup because the project does not expose a reusable deterministic hash helper at the application boundary and the query still compares the full normalized value.

Alias lookup uses the exact tenant-local alias key: `tenant_id`, `alias_type`, and `normalized_value`. `alias_value` is returned as stored evidence/display metadata but is not used for matching. Lookup returns the Resource directly referenced by the matching identifier or alias row; merge lineage is not followed and canonical resolution remains the separate `ResolveCanonicalResourceQuery`.

The SQLAlchemy adapter issues one projection query per exact lookup and selects only Resource plus matched identity scalar fields. Missing identities produce `EntityNotFoundError` in the handler, with wrong-tenant and absent rows indistinguishable. The lookup path performs no commit, flush, lock, mutation, N+1 Resource lookup, fuzzy matching, or generic search. Existing identifier indexes are `ix_resource_identifier_tenant_id_resource_id`, `ix_res_ident_tenant_type_hash`, `ix_res_ident_tenant_type_normalized`, `ix_resource_identifier_tenant_id_valid_to`, `uq_resource_identifier_current_value`, and `uq_resource_identifier_current_primary`. Existing alias indexes are `uq_resource_alias_tenant_alias_type_normalized_value`, `ix_resource_alias_tenant_resource_id`, `ix_resource_alias_tenant_alias_type`, and `ix_resource_alias_tenant_last_seen_at`. A future performance issue can consider an identifier partial current exact-lookup index including namespace if measured workloads need it.

Stage `03.2.5` hardening does not add a new Resource query capability. It regression-protects the existing Resource query-service surface: list pages, exact current identifier lookup, and exact alias lookup. The closeout integration coverage paginates a stable combined-filter dataset across three pages, verifies current organization/label/classification facts only, proves wrong-tenant rows do not leak, performs exact identifier and alias lookups for a listed Resource after Unit of Work exit, and verifies canonical resolution remains an explicit separate query.

Stage `03.2` index readiness is sufficient for correctness but not final performance tuning. Follow-up index candidates for measured high-volume workloads are `(tenant_id, created_at, id)` on `resource`, `(tenant_id, organization_id, resource_id) WHERE valid_to IS NULL AND is_primary = true AND ownership_role_id = owner-role-id` on `resource_ownership`, `(tenant_id, label_id, resource_id) WHERE valid_to IS NULL` on `resource_label`, `(tenant_id, classification_type_id, classification_value_id, resource_id) WHERE valid_to IS NULL` on `resource_classification`, and possibly an explicit current identifier exact-lookup index including namespace. No migration is required for Stage `03.2` closeout.

Stage `03.3.1` backs `GetResourceDetailsQuery(tenant_id, resource_id)` with `ResourceQueryService.get_resource_details(tenant_id, resource_id)`. The application result remains `ResourceDetailsResult` with `id`, `tenant_id`, `organization_id`, `resource_type_id`, `canonical_name`, `display_name`, `record_version`, `created_at`, `updated_at`, `state`, `identifiers`, `ownership`, `classifications`, `labels`, `aliases`, and `outgoing_merge`. The read model describes the requested stored Resource and does not automatically resolve ResourceMerge lineage; canonical resolution remains `ResolveCanonicalResourceQuery`.

Details current-fact predicates are: current state where `resource_state.valid_to IS NULL`; current primary owner where tenant/resource match, ownership role is the seeded `owner` role, `is_primary IS true`, and `valid_to IS NULL`; all current ownership rows where `valid_to IS NULL`; current labels where `resource_label.valid_to IS NULL`; all current classifications where `resource_classification.valid_to IS NULL`, including non-primary rows; current identifiers where `resource_identifier.valid_to IS NULL`; and aliases by tenant/resource because `resource_alias` is non-temporal. Historical temporal facts are excluded. Resources with no optional facts still return successfully with `None`/empty tuple fields.

Details collection ordering is explicit: ownership by `ownership_role_id`, primary before non-primary, `organization_id`, id; labels by label `key`, label `value`, `label_id`, assignment id; classifications by `classification_type_id`, `classification_value_id`, id; identifiers by `identifier_type_id`, namespace nulls first, namespace, `normalized_value`, id; aliases by `alias_type`, `normalized_value`, id. The SQLAlchemy adapter uses six fixed scalar SELECTs and no one-to-many cartesian join: core Resource plus current state/current primary owner/direct outgoing merge, current ownership, current labels, current classifications, current identifiers, and aliases. Query count is independent of fact collection size and no ORM graph is returned.
...
Stage `03.3.3` adds the separate direct one-hop relationship workflow `GetResourceRelationshipsQuery(tenant_id, resource_id)` through `ResourceQueryService.get_resource_relationships(tenant_id, resource_id)`. `GetResourceDetailsQuery` and `GetResourceHistoryQuery` remain unchanged and do not include relationship expansion. `ResourceRelationshipsResult` contains `resource_id`, `tenant_id`, and `relationships: tuple[ResourceRelationshipResult, ...]`. Each relationship item contains `id`, `relationship_type_id`, stored `source_resource_id`, stored `target_resource_id`, derived `direction`, `confidence_score`, `valid_from`, `source`, and `created_at`.

`ResourceRelationship` is temporal/current-versioned: it has `valid_from` and nullable `valid_to`, plus a current unique index on `(tenant_id, source_resource_id, target_resource_id, relationship_type_id) WHERE valid_to IS NULL`. The read model is current-only, so it filters `resource_relationship.valid_to IS NULL`; relationship history remains out of scope. Direction is relative to the requested Resource: `"outgoing"` when the requested Resource is the stored source and `"incoming"` when it is the stored target. Stored source and target ids are never reversed. Self-relationships are rejected by command validation and by the `source_resource_not_target_resource` database check.

The SQLAlchemy adapter uses exactly two scalar SELECTs for an existing Resource: one core Resource/existence query and one `resource_relationship` query with explicit tenant scope and `(source_resource_id = :resource_id OR target_resource_id = :resource_id)`. Missing or wrong-tenant Resources stop after the core SELECT. The relationship query returns all current direct rows touching the requested stored Resource, ordered by outgoing before incoming, then `relationship_type_id`, stored source id, stored target id, and relationship id. It does not load related Resource details, join relationship-type names, query `resource_merge`, deduplicate rows, add pagination, apply hidden `LIMIT`, use `OFFSET`, `COUNT`, or `DISTINCT`, or perform recursive/transitive traversal.

Relationship reads describe the requested stored Resource and do not automatically resolve merge lineage or combine canonical relationship sets. Fully materializing the current one-hop relationship set is the Stage `03.3.3` decision; pagination is deferred until measured cardinality requires a separate contract. Existing source-side indexes are `ix_resource_relationship_tenant_id_source_resource_id` and `ix_resource_relationship_tenant_source_type`; existing target-side indexes are `ix_resource_relationship_tenant_id_target_resource_id` and `ix_resource_relationship_tenant_target_type`. If measured plans show the current OR query needs more support, add future partial current indexes for source and target access paths. No migration is required for Stage `03.3.3`.

Stage `03.3.4` closes Block 03 for the Application Core & Persistence Layer after audit and validation. The completed scope is the Resource application command/read handler set, application query/result contracts, repository and query-service ports, SQLAlchemy repositories, `SQLAlchemyResourceQueryService`, `SQLAlchemyUnitOfWork`, persistence error translation, tenant-scoped read/write behavior, and regression documentation. This closeout adds no new product read capability and no migration. Details, history, relationships, and canonical resolution remain separate explicit contracts: details returns current operational state, history returns stored temporal fact sequences, relationships returns current direct one-hop edges, and canonical resolution alone traverses merge lineage.

Final confirmed Resource read query counts are: list page `1` SELECT, identifier lookup `1`, alias lookup `1`, details `6` for an existing Resource, history `6` for an existing Resource and `1` for missing/wrong tenant, relationships `2` for an existing Resource and `1` for missing/wrong tenant. Canonical resolution intentionally remains traversal-dependent because it walks direct outgoing merge edges with cycle and depth guards. Dense data tests protect fixed query counts for details, history, and relationships; the closeout integration matrix protects tenant isolation across list, identity lookup, alias lookup, details, history, relationships, and canonical resolution.
