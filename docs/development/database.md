...
The cursor is a position marker, not a snapshot. It does not include tenant or filter state; tenant isolation comes from the SQL predicate, so using a cursor with a different tenant cannot expose the previous tenant's rows. The current schema has tenant/filter indexes for Resource type and lifecycle-status lookups and ownership indexes for tenant/resource, tenant/organization, tenant/role, tenant/validity, tenant/resource/role, current uniqueness, and current-primary uniqueness. It has no composite `(tenant_id, created_at, id)` index for the default keyset order and no partial organization-filtered current-primary owner index. Add follow-up indexes before high-volume list usage depends on this path.
