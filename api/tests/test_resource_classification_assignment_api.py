...
from app.api.mappers import resource_classification_assigned_response
...
class RecordingAssignResourceClassificationHandler:
    def __init__(
        self,
        result: ResourceClassificationAssignedResult,
    ) -> None:
        self.result = result
        self.commands: list[AssignResourceClassificationCommand] = []
...
class RaisingAssignResourceClassificationHandler:
...
class RecordingAssignResourceOwnershipHandler:
    def __init__(self, result: ResourceOwnershipAssignedResult) -> None:
        self.result = result
...
@contextmanager
def _client_with_classification_override(handler: object) -> Iterator[TestClient]:
    sentinel = object()
    previous = app.dependency_overrides.get(
        get_assign_resource_classification_handler,
        sentinel,
    )
    app.dependency_overrides[get_assign_resource_classification_handler] = (
        lambda: handler
    )
    try:
...
def _assigned_result(
    *,
    resource_id: UUID,
    classification_id: UUID = UUID("0198a4a2-0000-7000-8000-000000000501"),
    classification_type_id: UUID = UUID(
        "0198a4a2-0000-7000-8000-000000000502"
    ),
    classification_value_id: UUID = UUID(
        "0198a4a2-0000-7000-8000-000000000503"
    ),
    is_primary: bool = True,
    valid_from: datetime = datetime.fromisoformat("2026-08-20T10:30:00+00:00"),
    source: str | None = "cmdb",
) -> ResourceClassificationAssignedResult:
    return ResourceClassificationAssignedResult(
...
def _post_classification(
    ...
    return client.post(
        f"/api/v1/tenants/{tenant_id}/resources/{resource_id}/classifications",
        json=_request_payload() if payload is None else payload,
    )


def test_exact_classification_assignment_endpoint_exists() -> None:
    operations = {
        (method, route.path)
...
def test_successful_assignment_maps_exact_command_and_returns_201() -> None:
    ...
    handler = RecordingAssignResourceClassificationHandler(
        _assigned_result(
            resource_id=resource_id,
            classification_id=classification_id,
            classification_type_id=classification_type_id,
    ...
    payload = {
        "classification_type_id": str(classification_type_id),
        "classification_value_id": str(classification_value_id),
        "is_primary": False,
    ...
    with _client_with_classification_override(handler) as client:
        response = _post_classification(client, tenant_id, resource_id, payload)
...
def test_request_schema_has_no_body_tenant_or_resource_ids() -> None:
    assert "tenant_id" not in AssignResourceClassificationRequest.model_fields
    assert "resource_id" not in AssignResourceClassificationRequest.model_fields
...
def test_classification_type_and_value_ids_are_preserved_without_api_resolution() -> None:
    tenant_id = uuid4()
    resource_id = uuid4()
    classification_type_id = uuid4()
    classification_value_id = uuid4()
    ...
    assert response.status_code == 201
    assert handler.commands[0].classification_type_id == classification_type_id
    assert handler.commands[0].classification_value_id == classification_value_id


def test_type_value_pair_is_passed_unchanged_without_compatibility_precheck() -> None:
    tenant_id = uuid4()
    resource_id = uuid4()
    classification_type_id = uuid4()
    classification_value_id = uuid4()
...
def test_naive_valid_from_is_transport_validation_error_and_handler_is_not_called() -> None:
    ...
    with _client_with_classification_override(handler) as client:
        response = _post_classification(client, uuid4(), uuid4(), payload)
...
def test_malformed_tenant_uuid_is_transport_validation_error() -> None:
    ...
    with _client_with_classification_override(handler) as client:
        response = _post_classification(client, "not-a-uuid", uuid4())
...
def test_malformed_resource_uuid_is_transport_validation_error() -> None:
    ...
    with _client_with_classification_override(handler) as client:
        response = _post_classification(client, uuid4(), "not-a-uuid")

    assert response.status_code == 422
    assert handler.commands == []


def test_malformed_classification_type_uuid_is_transport_validation_error() -> None:
    handler = RecordingAssignResourceClassificationHandler(
        _assigned_result(resource_id=uuid4())
    )
    payload = {**_request_payload(), "classification_type_id": "not-a-uuid"}
...
def test_malformed_classification_value_uuid_is_transport_validation_error() -> None:
    handler = RecordingAssignResourceClassificationHandler(
        _assigned_result(resource_id=uuid4())
    )
    payload = {**_request_payload(), "classification_value_id": "not-a-uuid"}
...
def test_missing_required_body_field_is_transport_validation_error() -> None:
    ...
    payload.pop("classification_value_id")
...
def test_response_mapping_preserves_null_source() -> None:
    ...
    with _client_with_classification_override(handler) as client:
        response = _post_classification(client, tenant_id, resource_id)
...
def test_classification_assigned_schema_and_mapper_match_application_result() -> None:
    resource_id = uuid4()
    classification_id = uuid4()
    classification_type_id = uuid4()
    classification_value_id = uuid4()
    ...
    response = resource_classification_assigned_response(result)

    assert list(ResourceClassificationAssignedResponse.model_fields) == [
        field.name for field in fields(ResourceClassificationAssignedResult)
    ]
    assert response == ResourceClassificationAssignedResponse(
        resource_id=resource_id,
        classification_id=classification_id,
        classification_type_id=classification_type_id,
        classification_value_id=classification_value_id,
...
def test_application_validation_error_uses_centralized_response() -> None:
    handler = RaisingAssignResourceClassificationHandler(
        ValidationError(
    ...
    with _client_with_classification_override(handler) as client:
        response = _post_classification(client, uuid4(), uuid4())

    assert response.status_code == 422
    assert response.json()["error"]["code"] == "validation_error"
    assert response.json()["error"]["details"] == [
        {"field": "classification_value_id", "message": "does not belong to type"}
    ]
...
def test_tenant_boundary_error_matches_not_found_envelope() -> None:
    missing_handler = RaisingAssignResourceClassificationHandler(
        EntityNotFoundError("missing")
    )
    boundary_handler = RaisingAssignResourceClassificationHandler(
        TenantBoundaryError("other tenant")
    )

    with _client_with_classification_override(missing_handler) as client:
        missing_response = _post_classification(client, uuid4(), uuid4())
    with _client_with_classification_override(boundary_handler) as client:
        boundary_response = _post_classification(client, uuid4(), uuid4())
...
def test_conflict_error_uses_centralized_response() -> None:
    handler = RaisingAssignResourceClassificationHandler(
        ConflictError("classification conflict", constraint="resource_classification_key")
    )
    ...
    assert "resource_classification_key" not in response.text


def test_concurrent_modification_error_uses_specific_centralized_response() -> None:
    handler = RaisingAssignResourceClassificationHandler(
        ConcurrentModificationError("classification changed")
    )
    ...
    assert "classification changed" not in response.text
...
def test_post_succeeds_with_only_classification_assignment_handler_override() -> None:
    tenant_id = uuid4()
    resource_id = uuid4()
    handler = RecordingAssignResourceClassificationHandler(
        _assigned_result(resource_id=resource_id)
...
def test_existing_write_routes_still_work_with_their_own_overrides() -> None:
    ...
    ownership_handler = RecordingAssignResourceOwnershipHandler(
        ResourceOwnershipAssignedResult(
            resource_id=resource_id,
            ownership_id=uuid4(),
    ...
    with _client_with_ownership_override(ownership_handler) as client:
