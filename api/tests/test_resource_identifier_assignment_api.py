...
from app.api.mappers import resource_identifier_assigned_response
...
class RaisingAssignResourceIdentifierHandler:
...
def _assigned_result(
    *,
    resource_id: UUID,
    identifier_id: UUID = UUID("0198a4a2-0000-7000-8000-000000000301"),
    identifier_type_id: UUID = UUID("0198a4a2-0000-7000-8000-000000000302"),
    original_value: str = " Example.COM ",
    normalized_value: str = "example.com",
    value_hash: str = "ABCDEF012345",
    namespace: str | None = "dns",
    is_primary: bool = True,
    valid_from: datetime = datetime.fromisoformat("2026-08-20T10:30:00+00:00"),
) -> ResourceIdentifierAssignedResult:
    return ResourceIdentifierAssignedResult(
...
def test_exact_identifier_assignment_endpoint_exists() -> None:
    operations = {
        (method, route.path)
...
def test_successful_assignment_maps_exact_command_and_returns_201() -> None:
    ...
    handler = RecordingAssignResourceIdentifierHandler(
        _assigned_result(
            resource_id=resource_id,
            identifier_id=identifier_id,
            identifier_type_id=identifier_type_id,
            original_value=" Azure VM:Prod ",
            normalized_value="azure-vm:prod",
            value_hash="ABCDEF012345",
            namespace=" cloud ",
            is_primary=False,
            valid_from=valid_from,
    ...
    with _client_with_identifier_override(handler) as client:
        response = _post_identifier(client, tenant_id, resource_id, payload)
...
def test_request_schema_has_no_body_tenant_or_resource_ids() -> None:
    assert "tenant_id" not in AssignResourceIdentifierRequest.model_fields
    assert "resource_id" not in AssignResourceIdentifierRequest.model_fields
...
def test_identifier_values_are_preserved_without_api_normalization_or_hashing() -> None:
    tenant_id = uuid4()
    resource_id = uuid4()
    handler = RecordingAssignResourceIdentifierHandler(
    ...
    assert response.status_code == 201
    assert handler.commands[0].original_value == " Example.COM "
    assert handler.commands[0].normalized_value == "example.com"
    assert handler.commands[0].value_hash == "ABCDEF012345"


def test_omitted_namespace_maps_to_none() -> None:
    tenant_id = uuid4()
    resource_id = uuid4()
    handler = RecordingAssignResourceIdentifierHandler(
        _assigned_result(resource_id=resource_id, namespace=None)
    )
    ...
    assert response.status_code == 201
    assert handler.commands[0].namespace is None


def test_explicit_empty_namespace_is_preserved() -> None:
    tenant_id = uuid4()
    resource_id = uuid4()
    handler = RecordingAssignResourceIdentifierHandler(
        _assigned_result(resource_id=resource_id, namespace="")
    )
    payload = {**_request_payload(), "namespace": ""}

    with _client_with_identifier_override(handler) as client:
        response = _post_identifier(client, tenant_id, resource_id, payload)

    assert response.status_code == 201
    assert handler.commands[0].namespace == ""
...
def test_naive_valid_from_is_transport_validation_error_and_handler_is_not_called() -> None:
    ...
    with _client_with_identifier_override(handler) as client:
        response = _post_identifier(client, uuid4(), uuid4(), payload)
...
def test_malformed_tenant_uuid_is_transport_validation_error() -> None:
    ...
    with _client_with_identifier_override(handler) as client:
        response = _post_identifier(client, "not-a-uuid", uuid4())
...
def test_malformed_resource_uuid_is_transport_validation_error() -> None:
    ...
    with _client_with_identifier_override(handler) as client:
        response = _post_identifier(client, uuid4(), "not-a-uuid")

    assert response.status_code == 422
    assert handler.commands == []


def test_malformed_identifier_type_uuid_is_transport_validation_error() -> None:
    handler = RecordingAssignResourceIdentifierHandler(
        _assigned_result(resource_id=uuid4())
    )
    payload = {**_request_payload(), "identifier_type_id": "not-a-uuid"}
...
def test_response_mapping_preserves_null_namespace() -> None:
    tenant_id = uuid4()
    resource_id = uuid4()
    handler = RecordingAssignResourceIdentifierHandler(
        _assigned_result(resource_id=resource_id, namespace=None)
    ...
    with _client_with_identifier_override(handler) as client:
        response = _post_identifier(client, tenant_id, resource_id)
...
def test_identifier_assigned_schema_and_mapper_match_application_result() -> None:
    resource_id = uuid4()
    identifier_id = uuid4()
    identifier_type_id = uuid4()
    ...
    response = resource_identifier_assigned_response(result)

    assert list(ResourceIdentifierAssignedResponse.model_fields) == [
        field.name for field in fields(ResourceIdentifierAssignedResult)
    ]
    assert response == ResourceIdentifierAssignedResponse(
        resource_id=resource_id,
        identifier_id=identifier_id,
        identifier_type_id=identifier_type_id,
        original_value=" Example.COM ",
...
def test_application_validation_error_uses_centralized_response() -> None:
    handler = RaisingAssignResourceIdentifierHandler(
        ValidationError(
            "invalid identifier",
            failures=(ValidationFailure("normalized_value", "must match hash"),),
        )
    )

    with _client_with_identifier_override(handler) as client:
        response = _post_identifier(client, uuid4(), uuid4())

    assert response.status_code == 422
    assert response.json()["error"]["code"] == "validation_error"
    assert response.json()["error"]["details"] == [
        {"field": "normalized_value", "message": "must match hash"}
    ]


def test_entity_not_found_error_uses_centralized_non_disclosing_response() -> None:
    handler = RaisingAssignResourceIdentifierHandler(EntityNotFoundError("missing"))
...
def test_tenant_boundary_error_matches_not_found_envelope() -> None:
    missing_handler = RaisingAssignResourceIdentifierHandler(
        EntityNotFoundError("missing")
    )
    boundary_handler = RaisingAssignResourceIdentifierHandler(
        TenantBoundaryError("other tenant")
    )

    with _client_with_identifier_override(missing_handler) as client:
        missing_response = _post_identifier(client, uuid4(), uuid4())
    with _client_with_identifier_override(boundary_handler) as client:
        boundary_response = _post_identifier(client, uuid4(), uuid4())
...
def test_conflict_error_uses_centralized_response() -> None:
    handler = RaisingAssignResourceIdentifierHandler(
        ConflictError("identifier conflict", constraint="resource_identifier_key")
    )
    ...
    assert "resource_identifier_key" not in response.text


def test_concurrent_modification_error_uses_specific_centralized_response() -> None:
    handler = RaisingAssignResourceIdentifierHandler(
        ConcurrentModificationError("identifier changed")
    )
    ...
    assert "identifier changed" not in response.text
...
def test_post_succeeds_with_only_identifier_assignment_handler_override() -> None:
    tenant_id = uuid4()
    resource_id = uuid4()
    handler = RecordingAssignResourceIdentifierHandler(
        _assigned_result(resource_id=resource_id)
